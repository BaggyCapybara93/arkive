...
impl<'a> FileManager<'a> {
    ...
    fn scan_unused_files(&self) -> Result<(), FileManagerError> {
        ...
        fn scan_directory_recursive(dir: &PathBuf, now: SystemTime, thirty_days: u64, settings: &FileManager<'_>) -> Result<(), FileManagerError> {
            for entry in fs::read_dir(dir)? {
                ...
                scan_file(&path, now, thirty_days, settings)?;
                
                if path.is_dir() {
                    scan_directory_recursive(&path, now, thirty_days, settings)?;
                }
            }
            Ok(())
        }
        
        scan_directory_recursive(&src.to_path_buf(), now, thirty_days, self)?;
    ...
    fn scan_and_remove_empty_dirs(&self) -> Result<(), FileManagerError> {
                ...
                Ok(entries) => {
                    for entry in entries {
                        ...
                        if entry_path.is_dir() {
                            find_empty_dirs_recursive(&entry_path, empty_dirs)?;
        ...
        find_empty_dirs_recursive(&src.to_path_buf(), &mut empty_dirs)?;
